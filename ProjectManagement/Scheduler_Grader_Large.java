...
public class Scheduler_Grader_Large {
    ...
    private static ArrayList<Long> user_time, job_time, project_time, rtc_time, job_report_time,
        user_report_time, flush_time;
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        if (!debug) {
            ...
            System.out.println("Job report average time(ms): " + calculateAverage(job_report_time)
                + " Total: " + calculateSum(job_report_time));
            System.out.println("User report average time (ms): "
                + calculateAverage(user_report_time) + " Total: " + calculateSum(user_report_time));
            System.out.println("Flush report average time (ms): " + calculateAverage(flush_time)
                + " Total: " + calculateSum(flush_time));
            System.out.println();
        }
    }
    ...
    private static void handle_cmd_query(String[] cmd) {
        ...
        switch (cmd[0]) {
            ...
            case "NEW_TOP":
                ...
                ArrayList<UserReport_> out_topuser =
                    scheduler_driver.timed_top_consumer(Integer.parseInt(cmd[1]));
    ...
    private static void validate_timed_report(ArrayList<JobReport_> out, String[] cmd) {
        ...
        switch (cmd[0]) {
            case "NEW_PROJECT":
                ...
                System.out.print(". Expecting Arrival time between: " + start_time + " and "
                    + end_time + " and belongs to: " + project_name);
                for (JobReport_ jobReport_ : out) {
                    if (jobReport_.arrival_time() < start_time
                        || jobReport_.arrival_time() > end_time
                        || jobReport_.project_name().compareToIgnoreCase(project_name.trim())
                            != 0) {
            ...
            case "NEW_USER":
                ...
                end_time = Integer.parseInt(cmd[3]);
                System.out.print(". Expecting Arrival time between: " + start_time + " and "
                    + end_time + " and belongs to: " + user_name);
            ...
            case "NEW_PROJECTUSER":
                ...
                System.out.print(". Expecting Arrival time between: " + start_time + " and "
                    + end_time + " and belongs to: " + project_name + ", " + user_name);
                for (JobReport_ jobReport_ : out) {
                    if (jobReport_.arrival_time() < start_time
                        || jobReport_.arrival_time() > end_time
            ...
            case "NEW_PRIORITY":
                ...
                //                System.out.print(". Expecting Priority: " + priority + "  or
                //                more");
                ////                for (JobReport_ jobReport_ : out) {
